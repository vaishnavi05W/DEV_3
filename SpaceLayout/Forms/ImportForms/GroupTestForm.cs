...
{
    public partial class GroupTestForm : Form
    {
        public GroupTestForm()
        {
            InitializeComponent();
        }

        private void GroupTestForm_Load(object sender, EventArgs e)
        {
			// Create the A group
			NGroup groupA = CreateGroup("A");
			groupA.Location = new NPointF(10, 10);
			Nfd.ActiveLayer.AddChild(groupA);

			// Create the B group
			NGroup groupB = CreateGroup("B");
			groupB.Location = new NPointF(10, 350);
			Nfd.ActiveLayer.AddChild(groupB);
